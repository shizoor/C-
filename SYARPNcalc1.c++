// Needs massive code tidy up, check for mismatched brackets, check for numbers with 2 decimal points, possibly add functions
	...
	for (i = 0; i < tokenstream.size(); i++) {
		...
		if (isanoperator(tokenstream.at(i)[0]) && !isaleftparenthesis(tokenstream.at(i)[0]) && !isarightparenthesis(tokenstream.at(i)[0])) {
			...
			while (((!operatorstack.empty() && isanoperator(operatorstack.at(0)[0])) && ((getprecedence(operatorstack.at(0)[0]) > getprecedence(tokenstream.at(i)[0]) || (getprecedence(operatorstack.at(0)[0]) == getprecedence(tokenstream.at(i)[0]) && getassociativity(tokenstream.at(i)) == "left") ))) && !isaleftparenthesis(operatorstack.back()[0])) {
				...
				cout << operatorstack.back() << " pushed to the output queue\n";
				cout << isaleftparenthesis(operatorstack.back()[0]) << endl;
				cout << isanoperator(operatorstack.back()[0]);
				cout << isanoperator(operatorstack.at(0)[0]);
				operatorstack.pop_back();

			}
			cout << tokenstream.at(i) << "pushed to the operator stack\n";
...
int main() {
	...
	while (1) {
		...
		if (int(inputstring.length()) > 0) { cout << rpnprocess(syaprocess(inputstring)); }
